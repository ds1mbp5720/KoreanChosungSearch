...
class KCS {
    ...
    companion object {
        ...
        */
            ...
            if (toCompare.length >= query.length && !(isOnlyCho(query).isNullOrEmpty())) { // 초성이 섞인 경우
                if (isOnlyCho(query).size != query.length) { // 초성, 글 혼용 검색
                    ...
                    if (!compareCho.contains(queryCho, true)) return false //초성 끼리 비교 다를경우 false 반환
                        ...
                        val substringCompare = toCompare.substring(
                            startIndex,
                            startIndex + query.length
                        ) // 초성일치부분만 자르기 (성능 고려)
                        for (i in letterPositionList.indices) {
                            // 글자 포함 안될경우 바로 false 반환
                            if (!substringCompare.contains(query[letterPositionList[i]], true)) return false
                            else matched = true
                        }
                    }
         ...
         */
        fun String.choIndexOf(query: String): Int {
            var startIndex = 0
            val letterPositionList = mutableListOf<Int>()
            if (this.length >= query.length && !(isOnlyCho(query).isNullOrEmpty())) {
                if (isOnlyCho(query).size != query.length) {
                    val queryChars = query.toCharArray()
                    val compareChars = this.toCharArray()
                    for (i in query.indices) {
                        val cho = getCho(query[i].toString()).uppercase()
                        if (isHangeul(
                                query[i].toString().single()
                            ) || !CHO.contains(query[i].toString())
                        ) {
                            letterPositionList.add(i)
                        }
                        queryChars[i] = cho.single()
                    ...
                    for (i in this.indices) {
                        val cho = getCho(this[i].toString()).uppercase()
                        compareChars[i] = cho.single()
                    }
                    val queryCho = String(queryChars)
                    val compareCho = String(compareChars)
                    if (!compareCho.contains(queryCho, true)) return -1
                    else {
                        startIndex = compareCho.indexOf(queryCho)
                        val substringCompare = this.substring(startIndex, startIndex + query.length)
                        for (i in letterPositionList.indices) {
                            if (!substringCompare.contains(
                                    query[letterPositionList[i]],
                                    true
                                )
                            ) return -1
                        }
                    ...
                    val compareChars = this.toCharArray()
                    for (i in this.indices) {
                        val cho = getCho(this[i].toString())
                        compareChars[i] = cho.single()
                    }
                    val compareReplaced = String(compareChars)
                    if (compareReplaced.contains(query, true)) {
                        startIndex = compareReplaced.indexOf(query)
                    }
                }
            } else if (this.contains(query, true)) {
                startIndex = this.indexOf(query)
            }
            return startIndex
        }
